      ...
      for( let id in idsToTweet ) {
        if( !tweeted.includes(idsToTweet[id]) ) {
          const bill = bills[id],
                link = bill.link,
                title = bill.title,
                truncTitle = title.substring(0, length) + "...",
                tweet = "Congress just introduced '" + truncTitle + "' " + link;
